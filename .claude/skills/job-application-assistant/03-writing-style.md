...
# Candidate-Specific Rules (José - from USI)

<!-- Regenerated by /sync-usi from USI meta/positioning.md and identity/summary.md. -->
<!-- Internal positioning guidance: informs how documents are written; never quote this section itself in outgoing documents. -->
<!-- Framework rules above this line are generic - do not edit them during sync. -->

## Headline rules (hard)
- Select headlines from the "Professional Headline Variants" in the USI summary block (`documents/usi/usi-content.md`); do not invent new ones. Default: **"Computer Engineering (MSc) | Process Engineering, AI Automation & Operational Excellence"**
- **"Graduate" only** when the posting explicitly targets graduate / very-junior (0-1 yr) hires

## QA framing suppression (applies to all roles)
- Never frame José as a QA Engineer. The Glartek experience is **delivery-infrastructure re-engineering and process improvement**; QA/testing is the domain where the work happened, not the identity
- QA-adjacent roles default to **Software Quality Engineer** framing, unless the role explicitly and exclusively targets a dedicated QA Engineer
- The Glartek title is always **"Software Quality Engineer"** - never "QA Engineer" or "QA Automation Engineer", and no "Internship" qualifier

## Primary target framings
Engineering Effectiveness / Developer Productivity and AI Automation / AI Integration are the two highest-priority role types. When a role is ambiguous, default to one of these two framings. The full role-tailoring map and per-role keyword banks live in `documents/usi/usi-strategy.md` - consult them when tailoring.

## Voice
- Direct, technical, grounded, first person - lead with the most role-relevant point, close with the systems-level breadth as context
- Quantified outcomes ride inside whole bullets (>50% / up to 20% / ~30% / 98%); keep the bullets that carry them
- Match keywords by surfacing skills and bullets that genuinely contain them - never keyword-stuff (framework rule) and never insert a keyword into an experience bullet the work doesn't support
- When mentioning agentic coding / AI tooling, name **Claude Code** explicitly (per CLAUDE.md)

## Facts discipline
- Every fact must trace to the USI corpus (via `documents/usi/` packs or `01-candidate-profile.md`). If a fact the role needs is missing, it goes into the USI corpus first (then `/sync-usi`) - never improvised into a document
