...
`/rank` extends this schema additively: ranked entries also carry `rank_score` (0–100 overall score), `rank_verdict` (fit band, e.g. "strong fit"), `rank_date` (ISO date of ranking), and `strengths`/`gaps` (1-3 verbatim bullets each, copied from the scoring agent's findings). The `status` field is set to `"ranked"`. Do not drop any of these fields when re-writing entries, with one exception: when an entry transitions to an **exit status** (`expired`, `skipped`, `archived`) its `strengths`/`gaps` arrays are pruned in that same write, per **Prune on exit** in `/rank` Step 4 - the justification prose is only meaningful while the job is still in the ranking pool. The scalar fields (`rank_score`, `rank_verdict`, `rank_date`) survive the prune, and the key always survives. Entries ranked before `strengths`/`gaps` existed simply lack them; readers tolerate their absence and never backfill by guessing.
