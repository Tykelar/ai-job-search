...
4. For each approved job: write the targeted CV (`CV_JoseHenriques_<company>_<role>.tex`) and cover letter (`CL_JoseHenriques_<company>_<role>.tex`) into the folder that already exists, one job finished before the next starts
